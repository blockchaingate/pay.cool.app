...
class _SelectPaymentBondViewState extends State<SelectPaymentBondView> {
  ...
  Future<void> bondApprove(Size size) async {
    var amount;
    String toAddress = Constants.bondAddressKANBAN;
    String? abiHex;
    ...
    if (selectedValueChain == 'ETH') {
      ...
      await paycoolService
          .encodeEthApproveAbiHex(context, Constants.bondAddress, amount)
          .then((value) async {
        setState(() {
          abiHex = Constants.bondApproveEthAbiCode + value.toString();
          toAddress = Constants.bondAddressETH;
        });
      });
      ...
      await paycoolService
          .encodeKanbanApproveAbiHex(context, Constants.bondAddress, 131073,
  ...
  Future<void> bondPurchase(Uint8List seed, Size size) async {
    var amount;
    String toAddress = Constants.bondAddress;
    String? abiHex;
    ...
    if (selectedValueChain == 'ETH') {
      ...
      await paycoolService
          .encodeEthPurchaseAbiHex(
              context, _emailController.text, Constants.bondAddress, amount)
          .then((value) async {
        setState(() {
